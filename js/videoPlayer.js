...
App.VideoPlayer = (function VideoPlayer() {
  ...
  function create(config) {
    ...
    var playerEl = config.playerEl || document.querySelector("#av-player");
    var controlsEl = config.controls;
    var subtitlesEl =
      config.subtitlesEl || document.querySelector("#subtitles");
    var timerEl = config.timerEl || document.querySelector(".time-info");
    ...
    function prepareAndPlay() {
      console.log("Prepare");
      webapis.avplay.prepareAsync(play, console.log(error));
    ...
    function toggleSubtitles() {
      ...
      console.log(subtitlesOn ? "subtitles On" : "subtitlesOff");
      subtitlesEl.innerText = "";
    ...
    function setBitrate(from, to, start, skip) {
      ...
      try {
        ...
        console.error("Failed setting bitrates: " + error.message);
      }
    }
    ...
    function setTrack(type, index) {
      try {
        ...
        console.error("Failed setting track: " + error.message);
      }
    }
    ...
    function getTracks() {
      ...
      try {
        ...
        console.error("Failed getting tracks: " + error.message);
      }
    ...
    function getProperties() {
      ...
      try {
        ...
        console.error("Failed getting properties: " + error.message);
      }
      ...
      try {
        ...
        if (config.set4KMode) {
          if (is4KSupported()) {
            ...
            console.log("4K is not supported");
          }
        }
        ...
        if (config.set8KMode) {
          if (is8KSupported()) {
            ...
            console.log("8K is not supported");
          }
        }
      } catch (error) {
        console.error(error.message);
    ...
    function updateTime() {
      ...
      timerEl.textContent =
        App.Utils.msToReadableTime(currentTime) +
        " / " +
